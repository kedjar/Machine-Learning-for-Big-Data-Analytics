...
# MAGIC # **Web Server Log Analysis**
# MAGIC 
# MAGIC This lab will demonstrate how easy it is to perform web server log analysis with Apache Spark.
# MAGIC 
# MAGIC Server log analysis is an ideal use case for Spark.  It's a very large, common data source and contains a rich set of information.  Spark allows you to store your logs in files on disk cheaply, while still providing a quick and simple way to perform data analysis on them.  This lab will show you how to use Apache Spark on real-world text-based production logs and fully harness the power of that data.  Log data comes from many sources, such as web, file, and compute servers, application logs, user-generated content,  and can be used for monitoring servers, improving business and customer intelligence, building recommendation systems, fraud detection, and much more.
...
# Helping variables
#days_with_hosts_row_object = daily_hosts_df.select(daily_hosts_df['day']).collect()
#hosts_row_object = daily_hosts_df.select(daily_hosts_df['count']).collect()

#days_with_hosts = [None]*len(days_with_hosts_row_object)
#hosts = [None]*len(days_with_hosts_row_object)
#for index in range(1,len(days_with_hosts_row_object)+1):
#  days_with_hosts[index-1] = days_with_hosts_row_object[index-1][0]
#  hosts[index-1]=hosts_row_object[index-1][0]

#print(days_with_hosts)
#print(hosts)

# Another option (Better?)
days_with_hosts = (daily_hosts_df
                  .rdd.map(lambda r: (r[0]))
                  .collect()
                   )
hosts = (daily_hosts_df
                  .rdd.map(lambda r: (r[1]))
                  .collect()
                   )
...
total_req_per_day_df = (logs_df
                        .select(logs_df['host'], dayofmonth(logs_df['time'])
                                .alias('day')).groupBy('day')
                        .count()
                        .sort('day'))

avg_daily_req_per_host_df = (total_req_per_day_df
                             .join(daily_hosts_df, total_req_per_day_df['day']==daily_hosts_df['day'])
                             .rdd.map(lambda x: (x[0],x[1]/x[3]))
                             .toDF(['day','avg_reqs_per_host_per_day'])
                            )

print 'Average number of daily requests per Hosts is:\n'
avg_daily_req_per_host_df.show()

# Caching avg_daily_req_per_host_df
...
# TODO: Replace <FILL IN> with appropriate code
#Helping variables
#days_with_avg_row_object = (avg_daily_req_per_host_df.select(avg_daily_req_per_host_df['day']).collect())
#avgs_row_object = (avg_daily_req_per_host_df.select(avg_daily_req_per_host_df['avg_reqs_per_host_per_day']).collect())

#days_with_avg = (avg_daily_req_per_host_df.<FILL IN>)
#avgs = (avg_daily_req_per_host_df.<FILL IN>)
#days_with_avg = [None]*len(days_with_avg_row_object)
#avgs = [None]*len(days_with_avg_row_object)
#for index in range(1, len(days_with_avg_row_object)+1):
#  days_with_avg[index-1] = days_with_avg_row_object[index-1][0]
#  avgs[index-1]=avgs_row_object[index-1][0]

#print(days_with_avg)
#print(avgs)

# Another option (Better?)
days_with_avg = (avg_daily_req_per_host_df
                  .rdd.map(lambda r: (r[0]))
                  .collect()
                   )
avgs = (avg_daily_req_per_host_df
                  .rdd.map(lambda r: (r[1]))
                  .collect()
                   )
...
top_20_not_found_df = (not_found_paths_df
                       .groupBy(not_found_paths_df['path'])
                       .count()
                       .sort('count', ascending=False)
                      )
...
errors_by_date_sorted_df = (not_found_df
                            .select(dayofmonth(not_found_df['time']).alias('day'))
                            .groupBy('day')
                            .count()
                            .sort('day')
                           )
...
# Caching errors_by_date_sorted_df
...
# TODO: Replace <FILL IN> with appropriate code
# Helping variables
#days_with_errors_404_row_object = errors_by_date_sorted_df.select(errors_by_date_sorted_df['day']).collect()
#errors_404_by_day_row_object = errors_by_date_sorted_df.select(errors_by_date_sorted_df['count']).collect()

#days_with_errors_404 = [None]*len(days_with_errors_404_row_object)
#errors_404_by_day = [None]*len(days_with_errors_404_row_object)
#for index in range(1,len(days_with_errors_404_row_object)+1) :
#  days_with_errors_404[index-1] = days_with_errors_404_row_object[index-1][0]
#  errors_404_by_day[index-1] = errors_404_by_day_row_object[index-1][0]

# Another option (Better?)
days_with_errors_404 = (errors_by_date_sorted_df
                        .rdd.map(lambda r: (r[0]))
                        .collect()
                       )
errors_404_by_day = (errors_by_date_sorted_df
                        .rdd.map(lambda r: (r[1]))
                        .collect()
                       )
...
hour_records_sorted_df = (not_found_df
                          .select(not_found_df['host'], hour(not_found_df['time']).alias('hour'))                          
                          .groupBy('hour')
                          .count()
                          .sort('hour'))
...
# Caching hour_records_sorted_df
...
# TODO: Replace <FILL IN> with appropriate code
# Helping variables
#hours_with_not_found_row_object = hour_records_sorted_df.select('hour').collect()
#not_found_counts_per_hour_row_object = hour_records_sorted_df.select('count').collect()

#hours_with_not_found = [None]*len(hours_with_not_found_row_object)
#not_found_counts_per_hour = [None]*len(hours_with_not_found_row_object)

#for index in range(1,len(hours_with_not_found_row_object)+1) :
#  hours_with_not_found[index-1] = hours_with_not_found_row_object[index-1][0]
#  not_found_counts_per_hour[index-1] = not_found_counts_per_hour_row_object[index-1][0]

# Another option (Better?)
hours_with_not_found = (hour_records_sorted_df
                        .rdd.map(lambda r: (r[0]))
                        .collect()
                       )
not_found_counts_per_hour = (hour_records_sorted_df
                        .rdd.map(lambda r: (r[1]))
                        .collect()
                       )
