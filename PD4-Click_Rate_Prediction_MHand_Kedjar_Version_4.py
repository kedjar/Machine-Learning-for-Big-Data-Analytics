...
# MAGIC 
# MAGIC ** This notebook will cover: **
# MAGIC 
...
# MAGIC 
# MAGIC 
# MAGIC > * I recommend using a Spark 2.1 (Auto-updating, Scala 2.11) cluster version
...
import numpy as np
# ALEKOE from pyspark.mllib.linalg import SparseVector
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    """
    one_hot_features = (SparseVector(num_ohe_feats, 
                                     [(ohe_dict_broadcast.value[feat_tuples], 1.0) 
                                      for feat_tuples in raw_feats])
                       )
...
sample_one_ohe_feat = (one_hot_encoding(sample_one, 
                                        sample_ohe_dict_manual_broadcast, 
                                        num_sample_ohe_feats))
...
# ALEKOE from pyspark.mllib.linalg import VectorUDT
...
sample_distinct_feats_df = (sample_data_df
                              .select(explode(sample_data_df.features))
                              .distinct()
                           )
...
def create_one_hot_dict(input_df):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    input_distinct_features_df = (input_df
                                    .select(explode(input_df.features))
                                    .distinct()
                                 )
    input_ohe_dict_df = (input_distinct_features_df
                                    .rdd
                                    .map(lambda r: tuple(r[0]))
                                    .zipWithIndex()
                                    .collectAsMap()
...
Test.assertEquals(n_train, 80115, 'incorrect value for n_train')
Test.assertEquals(n_val, 9955, 'incorrect value for n_val')
Test.assertEquals(n_test, 9930, 'incorrect value for n_test')
...
def parse_raw_df(raw_df):
    ...
    """
    parsed_feature_data_df = (raw_df.select(split(raw_df['text'], ',').getItem(0).cast('float').alias('label'),
                                            parse_point_udf(raw_df['text']).alias('feature'))
                             ) 
    parsed_feature_data_df.cache()
    
    return parsed_feature_data_df
...
Test.assertEquals(num_categories[2][1], 1720, 'incorrect implementation of parse_point or parse_raw_df')
...
Test.assertEquals(num_ctr_ohe_feats, 233618, 'incorrect number of features in ctr_ohe_dict')
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    """   
    one_hot_features = (SparseVector(num_ohe_feats, 
                                     [(ohe_dict_broadcast.value[feat_tuples], 1.) 
                                      for feat_tuples in raw_feats if ohe_dict_broadcast.value.has_key(feat_tuples)]
                                    )
                       )
    return one_hot_features
...
Test.assertTrue(np.allclose(lr_model_basic.intercept,  -1.22762492787), 'incorrect value for model intercept')
Test.assertTrue(np.allclose(sorted_coefficients,
                           [-0.11519510891031376, -0.11034521078486437, -0.10881778122436737, -0.10880528872597497, -0.10483538097546272]
),
...
def add_log_loss(df):
    ...
    """
    log_loss_df =(df.select("*", when((df.label == 1) & (df.p == 0),                 0. - log(df.p + epsilon))
                           .when((df.label == 1) & (df.p == 1),                 0. - log(df.p - epsilon))
                           .when((df.label == 1) & ((df.p != 1) & (df.p != 0)), 0. - log(df.p))
                           .when((df.label == 0) & (df.p == 0),                 0. - log(1. - (df.p + epsilon)))
                           .when((df.label == 0) & (df.p == 1),                 0. - log(1. - (df.p - epsilon)))
                           .otherwise(                                          0. - log(1. - df.p))
                           .alias('log_loss')
                           )             
                   )   
    return log_loss_df
...
expected_frac = 0.226736566186
expected_log_loss = 0.535302929556
...
expected_log_loss = 0.474423503078
...
expected_val_base = 0.534666713098
...
expected_val_model_basic = 0.477804307905
...
# TODO: Replace <FILL IN> with appropriate code
# ALEKOE from pyspark.mllib.linalg import Vectors
...
def add_hashed_features(df):
    ...
    """
    hashed_features_df = (df
                          .select('label', tuples_to_hash_features_udf('feature').alias('features'))
                         )
    
    return hashed_features_df 
...
expected_validation_sum = 6661495
...
def vector_feature_sparsity(sparse_vector):
    ...
    """
    sparcity_ratio = len(sparse_vector.indices) * 1.0 / len(sparse_vector)
    
    return sparcity_ratio
...
def get_sparsity(df):
    ...
    """
    ave_sparcity_feat = (df
                          .select(feature_sparsity_udf('features'))
                          .groupBy()
                          .mean()
                          .collect()
                        )[0][0]
    return ave_sparcity_feat
...
# TEST Sparsity (5d)
expected_ohe = 1.66939191329e-04
...
lr_hash = (LogisticRegression(maxIter = max_iter,
                              regParam = reg_param,
                              elasticNetParam = elastic_net_param,
                              standardization = standardization)
          )
...
expected_test_baseline = 0.5346241446
