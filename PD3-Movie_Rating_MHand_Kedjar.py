...
print 'raw_ratings_df:'
raw_ratings_df.show(3, truncate=False)
print 'ratings_df:'
ratings_df.show(3, truncate=False)
print 'raw_movies_df:'
raw_movies_df.show(3, truncate=False)
print 'movies_df:'
movies_df.show(3, truncate=False)
...
#movie_names_df = movies_df.select(movies_df['title'], movies_df['ID'].alias('movieId'))
movie_names_df = movie_ids_with_avg_ratings_df.join(movies_df.select(movies_df['title'], movies_df['ID'].alias('movieId')), "movieId")
print 'movie_names_df:'
movie_names_df.show(3, truncate=False)
movie_names_with_avg_ratings_df = movie_names_df.select('movieId', 'title', 'count', 'average').sort('average', ascending=False)
...
#avg_rating_df = training_df.groupBy('movieId').agg(F.count(training_df.rating).alias("count"), F.avg(training_df.rating).alias("average"))
#avg_rating_df = training_df.agg(F.sum(training_df.rating)/training_df.count())
avg_rating_df = training_df.agg(F.avg(training_df.rating))
avg_rating_df.show(3)
#avg_rating_df2 =  avg_rating_df.agg(F.sum(avg_rating_df.average)/avg_rating_df.count())
#avg_rating_df2.show(3)
#avg_rating_df.count()
#ratings_df.groupBy('movieId').agg(F.count(ratings_df.rating).alias("count"), F.avg(ratings_df.rating).alias("average"))
...
display(movies_with_500_ratings_or_more.orderBy(movies_with_500_ratings_or_more['average'].desc()).take(400))
...
my_rated_movies = [
  ...
  (my_user_id, 1209, 5.0 ),
  (my_user_id, 1201, 5.0 ),
  (my_user_id, 26131, 5.0 ),
  (my_user_id, 30749, 5.0 ),
  (my_user_id, 27611, 5.0 ),
  (my_user_id, 1291, 5.0 )
...
predicted_ratings_df.show(1)
movie_names_with_avg_ratings_df.show(1)
...
predicted_with_counts_df = predicted_ratings_df.join(movie_names_with_avg_ratings_df.select(movie_names_with_avg_ratings_df['movieId'], movie_names_with_avg_ratings_df['title'], movie_names_with_avg_ratings_df['count']), "movieId")
...
#predicted_highest_rated_movies_df = predicted_with_counts_df.filter(predicted_with_counts_df['count']>=75)
